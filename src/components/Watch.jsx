...
export default function Watch({time, mobileMode})
{
    const markers = [];
    ...
    for (let i = 0; i < 60; i++)
    {
        markers.push(<div 
                        ...
                        style={{transform: ` rotateZ(${i * 6}deg) translateY(${mobileMode ? "-40vw" : "-170px"})`}}
                    ></div>)
    }
    ...
    return (
        <div role="timer">
            ...
            <div
                data-testid="hand"
                className="hand"
                style={{transform: ` rotateZ(${handAngle}turn) translateY(${mobileMode ? "-45vw" : "-185px"})`}}
            ></div>
